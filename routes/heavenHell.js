...
async function uploadUserCardBgToS3(file, folderPath = "images/question11/CardBg") {
...
router.post('/create', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            ...
            const { filename, url } = await uploadUserCardBgToS3(req.file, "images/question11/CardBg");
        ...
        heavenHellControllers.Create(req, res, next);
    } catch (error) {
        console.error('Error during heavenHell create:', error);
        res.status(500).json({
...
router.post('/read', heavenHellControllers.Read);

router.patch('/update/:id', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            // Get current HeavenHell record before updating
            const { id } = req.params;
            const currentHeavenHell = await HeavenHell.findById(id); // Adjust based on your model method
            
            if (currentHeavenHell && currentHeavenHell.CardBg) { // Adjust field name as per your schema
                // Delete old file from S3
                const bucketName = process.env.AWS_BUCKET_NAME;
                await deleteFromS3(currentHeavenHell.CardBg, bucketName);
            }
        ...
        heavenHellControllers.Update(req, res, next);
    } catch (error) {
        console.error('Error during heavenHell update:', error);
        res.status(500).json({
            error: 'CardBg upload failed',
            details: error.message
        });
...
router.delete('/delete/:id', async (req, res, next) => {
    try {
        // Get current HeavenHell record before deleting
        const { id } = req.params;
        const currentHeavenHell = await HeavenHell.findById(id); // Adjust based on your model method
        ...
        heavenHellControllers.Delete(req, res, next);
    } catch (error) {
        console.error('Error during heavenHell delete:', error);
        res.status(500).json({
            error: 'HeavenHell deletion failed',
            details: error.message
        });
    }
});
