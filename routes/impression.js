...
async function uploadUserCardBgToS3(file, folderPath = "images/question13/CardBg") {
...
router.post('/cardbg/create', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            const { filename, url } = await uploadUserCardBgToS3(req.file, "images/question13/CardBg");
            req.file.filename = filename;
            req.file.s3Url = url;
        }
        impressionCategory.Create(req, res, next);
    } catch (error) {
...
router.post('/cardbg/read', impressionCategory.Read);
...
router.patch('/cardbg/update/:id', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            ...
            const { id } = req.params;
            const currentCardBg = await ImpressionCardBg.findById(id); 
        ...
        impressionCategory.Update(req, res, next);
    } catch (error) {
...
router.delete('/cardbg/delete/:id', async (req, res, next) => {
    try {
        ...
        impressionCategory.Delete(req, res, next);
    } catch (error) {
