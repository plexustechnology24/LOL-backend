...
async function uploadUserCardBgToS3(file, folderPath = "images/question7/CardBg") {
...
router.post('/cardbg/create', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            const { filename, url } = await uploadUserCardBgToS3(req.file, "images/question7/CardBg");
            req.file.filename = filename;
            req.file.s3Url = url;
        }
        friendCategory.Create(req, res, next);
    } catch (error) {
...
router.post('/cardbg/read', friendCategory.Read);
...
router.patch('/cardbg/update/:id', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            ...
            const { id } = req.params;
            const currentCardBg = await FriendCardBg.findById(id); // Adjust based on your model method
        ...
        friendCategory.Update(req, res, next);
    } catch (error) {
...
router.delete('/cardbg/delete/:id', async (req, res, next) => {
    try {
        ...
        friendCategory.Delete(req, res, next);
    } catch (error) {
