...
async function uploadUserCardBgToS3(file, folderPath = "images/question12/CardBg") {
...
router.post('/cardbg/create', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            const { filename, url } = await uploadUserCardBgToS3(req.file, "images/question12/CardBg");
            req.file.filename = filename;
            req.file.s3Url = url;
        }
        reputationCategory.Create(req, res, next);
    } catch (error) {
...
router.post('/cardbg/read', reputationCategory.Read);
...
router.patch('/cardbg/update/:id', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            ...
            const { id } = req.params;
            const currentCardBg = await ReputationCardBg.findById(id); 
        ...
        reputationCategory.Update(req, res, next);
    } catch (error) {
...
router.delete('/cardbg/delete/:id', async (req, res, next) => {
    try {
        ...
        reputationCategory.Delete(req, res, next);
    } catch (error) {
