...
async function uploadUserCardBgToS3(file, folderPath = "images/question9/CardBg") {
...
router.post('/create', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            ...
            const { filename, url } = await uploadUserCardBgToS3(req.file, "images/question9/CardBg");
        ...
        bluffControllers.Create(req, res, next);
    } catch (error) {
        console.error('Error during bluff create:', error);
        res.status(500).json({
...
router.post('/read', bluffControllers.Read);

router.patch('/update/:id', upload.single("CardBg"), async (req, res, next) => {
    try {
        if (req.file) {
            // Get current Bluff record before updating
            const { id } = req.params;
            const currentBluff = await Bluff.findById(id); // Adjust based on your model method
            
            if (currentBluff && currentBluff.CardBg) { // Adjust field name as per your schema
                // Delete old file from S3
                const bucketName = process.env.AWS_BUCKET_NAME;
                await deleteFromS3(currentBluff.CardBg, bucketName);
            }
        ...
        bluffControllers.Update(req, res, next);
    } catch (error) {
        console.error('Error during bluff update:', error);
        res.status(500).json({
            error: 'CardBg upload failed',
            details: error.message
        });
...
router.delete('/delete/:id', async (req, res, next) => {
    try {
        // Get current Bluff record before deleting
        const { id } = req.params;
        const currentBluff = await Bluff.findById(id); // Adjust based on your model method
        ...
        bluffControllers.Delete(req, res, next);
    } catch (error) {
        console.error('Error during bluff delete:', error);
        res.status(500).json({
            error: 'Bluff deletion failed',
            details: error.message
        });
    }
});
